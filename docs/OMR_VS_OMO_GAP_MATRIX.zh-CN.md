...
| 任务事件记录 | 已有 OMR 生命周期 JSONL（夹具开始/完成/失败）；宿主事件仍通过 `--events` 接入 | 有任务、Hook、后台通知链 | P0 |

### 3.2 Agent 编排

| 能力 | OMR 当前状态 | OMO 对应能力 | 优先级 |
|---|---|---|---|
| 专用 OMR Agent | 只有 `omr-explore` | Sisyphus、Prometheus、Oracle、Librarian、Explore、Frontend 等 | P1 |
...
| Research Agent | 没有 OMR 版本 | Librarian/Research 类 Agent | P1 |
