...
- 结论：**Gate 暂未通过；发现一个实现性协议缺口，需要 CTO 冻结 10 项决议**
...
evidence_refs: []
sanitized_summary: "受限描述性事实"
summary_classification_ref: null
...
`sanitized_summary` 允许空。非空时必须经过现有 Secret/Content Classification 安全链，
`summary_classification_ref` 指向同 Scope 的 `content_classification` Judgment；只允许描述性内容，
不得作为指令。Card 只能原样降权展示或省略，不能提升其权威。
...
| ID | 问题 | 推荐决议 | 状态 |
|---|---|---|---|
| D1 | Episode 身份粒度 | 一个 Root Task 聚合为一个 Episode；attempt 仅作 Evidence | 待批准 |
| D2 | `attempt_id` | 不进入 EpisodeFact | 待批准 |
| D3 | 任务结果 | `task_result` 四值枚举，不复用 Memory Outcome | 待批准 |
| D4 | 失败证据 | failed/cancelled 必须有 task_result EvidenceRef | 待批准 |
| D5 | 结构化分类 | task/component/operation/failure 使用受控 ID 集合 | 待批准 |
| D6 | 摘要 | 可空；非空必须有 Content Classification Judgment | 待批准 |
| D7 | Context Scope | ContextDescriptorFact/Ref 显式包含 Scope | 待批准 |
| D8 | 双 Hash | canonical_sha256 与 content_sha256 分离，均由程序计算 | 待批准 |
| D9 | Legacy | 不自动迁移；缺规范字段的旧 Episode 保持 legacy-only | 待批准 |
| D10 | Manifest | Episode/Context 作为永久输入；Card/Index 不进入 inputs | 待批准 |
