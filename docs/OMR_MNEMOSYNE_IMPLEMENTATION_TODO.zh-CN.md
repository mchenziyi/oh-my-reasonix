...
- WEB-03：🟡 管理动作协议、严格校验、二次确认 CLI、loopback API 和 `/manager` 页面已实现（含带 basis refs 的 unfreeze、固定时间点 Lifecycle/Health/Usage/Pinned/Frozen/Archived 治理标记/关系展示与审计入口）；隔离临时项目已完成真实进程 Freeze/幂等 smoke，待浏览器/桌面联调；
