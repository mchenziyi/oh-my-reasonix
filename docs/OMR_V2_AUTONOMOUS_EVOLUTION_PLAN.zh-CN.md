...
> **Archived（已归档）**：本计划对应的工作已全部交付（v2.0.0 MVP、v2.0.1 Promotion Gate、v2.0.2 观察期报告、v2.0.3 经验包导入导出，以及 LP-01～LP-06 本地增强至 v2.0.9）。本文保留作为审计与追溯依据。

> 后续版本：v2.0.1 Promotion Gate 已交付；v2.0.2 观察期指标与历史报告、v2.0.3 经验包导入导出按独立版本推进。

> 状态：EV-MVP-00～10、EV-POST-01～03 已完成；OMR 可独立实现的自进化能力已完成。
> 当前版本：OMR v2.0.9（LP-01～LP-06 全部交付）；当前可用能力见 [当前可用能力矩阵](OMR_CAPABILITY_MATRIX.zh-CN.md)
