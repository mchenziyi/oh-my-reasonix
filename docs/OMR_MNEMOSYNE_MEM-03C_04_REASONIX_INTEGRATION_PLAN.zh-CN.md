...
`context` 是唯一允许解析 CURRENT 的入口：它在一次命令内把当前 Episodic Generation 固定成完整
...
如果当前 Generation 不是 `mnemosyne-episodic-compiler/1`，`context` 返回 `unavailable`，不把
Memory OKF Generation 冒充为 Episodic Generation，也不自动切换或创建 Generation。

## 三、Reasonix Profile 工作流
