...
- 不要在 OMR 中实现 RX-01～RX-06；
- 不要新增 `Runner + 文件系统回退`；
- 不要扫描 `~/.reasonix/projects`、goal-state、events 或 lock 文件；
- 不要以“宿主 CLI 不支持时返回空列表”伪造 Hook、Task 或 Session 能力；
- 不要把 `session events/results` 当作 RX 接口的替代品；
- 不要把未经过真实客户端验证的功能标记为完成。

RX-01～RX-06 是 Reasonix 宿主的前置能力，不属于本轮 OMR 开发任务。当前只
允许完成下面的 OMR-FIX-01～OMR-FIX-11。
