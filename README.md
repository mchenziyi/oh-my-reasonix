...
- **v2.0.0 自动自进化**：OMR 将作为 Reasonix 的项目级外置策略大脑，自动收集执行经验、识别重复模式、生成并验证 OMR 策略提案，通过项目级 Overlay 审批生效和回滚；Reasonix 仍负责 Agent 推理与执行。EV-MVP-00～08 自动化链路已完成，真实客户端联调尚未开始，详见 [v2.0.0 自动自进化开发计划](docs/OMR_V2_AUTONOMOUS_EVOLUTION_PLAN.zh-CN.md)；
